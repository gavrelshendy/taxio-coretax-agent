    ...
    return `(() => {
      ...
      function choose(btn){
        ...
        box.innerHTML='<div style="font-weight:700;margin-bottom:10px">Unduh SPT + Lampiran</div>'+
          '<label style="display:block;font-weight:600;margin-bottom:4px">Mode lampiran</label>'+
          '<select id="__ca_mode" style="width:100%;border:1px solid #94a3b8;border-radius:7px;padding:8px 9px;margin-bottom:10px;background:#fff;color:#1a202c">'+
            '<option value="print">Print (cepat, data dibatasi)</option><option value="full">Lengkap (semua baris)</option>'+
            (t&&t.kind==='article-21-26-tax-return'?'<option value="confidential">Confidential (ringkasan)</option>':'')+'</select>'+
          '<label style="display:block;font-weight:600;margin-bottom:4px">Hasil PDF</label>'+
          '<select id="__ca_output" style="width:100%;border:1px solid #94a3b8;border-radius:7px;padding:8px 9px;margin-bottom:11px;background:#fff;color:#1a202c"><option value="combined">Gabungan</option><option value="separate">Terpisah</option></select>'+
          ...
          '<div style="font-size:10.5px;color:#64748b;margin-top:7px;line-height:1.35">Mode Lengkap dapat memerlukan waktu lebih lama bila datanya ribuan baris.</div>';
        document.documentElement.appendChild(box);
        box.querySelector('#__ca_start').onclick=()=>{const mode=box.querySelector('#__ca_mode').value,output=box.querySelector('#__ca_output').value;box.remove();run(btn,mode,output)};
      }
      function ensure(){
