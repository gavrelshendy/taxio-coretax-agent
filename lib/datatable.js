...
async function isNextPageDisabled(page) {
    ...
    try {
        ...
        return await nextBtn.evaluate(el => el.classList.contains('p-disabled'));
    } catch (e) {
        return true; // can't find/read the button - treat as "no more pages" rather than loop forever
    }
}
async function goToNextPage(page) {
    const nextBtn = page.locator('.p-paginator-next').last();
    await nextBtn.evaluate(el => el.click()).catch(() => nextBtn.click({ timeout: 3000, force: true, noWaitAfter: true }));
...
async function downloadViaClick(page, btnLocator, downloadDir, targetPath, timeoutMs) {
    ...
    try {
        const [download] = await Promise.all([
            ...
            btnLocator.evaluate(el => el.click()).catch(() => btnLocator.click({ noWaitAfter: true, timeout: 5000, force: true }))
        ]);
