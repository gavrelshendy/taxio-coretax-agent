/* Coretax Agent - renders a standalone HTML string (Coretax's "View Receipt"/BPE content, read
   ...
   channel:'chrome' (CONFIRMED LIVE BUG 2026-07-30, real user report): a plain chromium.launch()
   with no channel drives Playwright's OWN separately-downloaded browser build, cached under
   %LOCALAPPDATA%\ms-playwright\ - a path completely outside node_modules, so pkg's asset
   bundling (playwright/playwright-core's npm package files only) never includes it. On a fresh
   machine, or after a Playwright version bump changes which browser revision it expects, that
   binary simply isn't there and launch() throws "Executable doesn't exist ... npx playwright
   install" - which end users obviously can't/shouldn't have to run for a packaged .exe. Real
   Chrome (channel:'chrome') supports headless:true too and exposes the exact same CDP-based
   page.pdf() capability - using it here instead means BPE rendering depends on nothing but the
   same already-installed Chrome every other part of this app already requires to run at all. */
const { chromium } = require('playwright');

let browserPromise = null;
function getBrowser() {
    if (!browserPromise) browserPromise = chromium.launch({ channel: 'chrome', headless: true });
