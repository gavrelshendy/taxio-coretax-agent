...
const watchdog = require('./coretax-watchdog');
...
async function hideWindowOffscreen(page) {
    try {
        // String literal, not a function reference - see revealWindow()'s comment for why.
        await page.evaluate('(function(){ window.moveTo(-32000, -32000); })()').catch(() => {});
...
async function revealWindow(page) {
    try {
        // MUST be a string, not a function reference - pkg's bytecode compilation mangles
        // Function.toString() in the packaged exe, which page.evaluate(fn) relies on to
        // serialize it. A function reference here silently throws "Passed function is not
        // well-serializable!" in the packaged build while working fine under plain `node .`
        // (confirmed live 2026-07-27) - matches the same string-literal pattern already used
        // for evaluate() calls elsewhere in this codebase for the same reason.
        await page.evaluate('(function(){ window.moveTo(0, 0); window.resizeTo(screen.availWidth, screen.availHeight); })()').catch((e) => log('[revealWindow] moveTo/resizeTo gagal: ' + e.message));
        const session = await page.context().newCDPSession(page).catch((e) => { log('[revealWindow] newCDPSession gagal: ' + e.message); return null; });
        if (session) {
            const windowId = (await session.send('Browser.getWindowForTarget')).windowId;
            // CDP refuses to jump straight from the off-screen/minimized state hideWindowOffscreen
            // leaves it in to 'maximized' in one call ("restore it to normal state first" -
            // confirmed live 2026-07-27) - restore to 'normal' first as its own separate call.
            await session.send('Browser.setWindowBounds', { windowId, bounds: { windowState: 'normal' } })
                .catch((e) => log('[revealWindow] restore-normal gagal: ' + e.message));
            await session.send('Browser.setWindowBounds', { windowId, bounds: { windowState: 'maximized' } })
                .catch((e) => log('[revealWindow] setWindowBounds gagal: ' + e.message));
        } else {
            log('[revealWindow] tidak ada sesi CDP - window mungkin tetap di luar layar.');
        }
        await page.bringToFront().catch((e) => log('[revealWindow] bringToFront gagal: ' + e.message));
        bringChromeToFrontOnWindows();
    } catch (e) {
        log('[revealWindow] gagal total: ' + e.message);
    }
}

function _setupBlockedHitTracking(context, page, picId) {
    try {
        context.exposeFunction('__ca_onBlockedHit', (url) => {
            log('[PERINGATAN KEAMANAN] Akun Terbatas mencoba mengakses URL diblokir: ' + url + ' (PIC: ' + picId + ')');
        }).catch(() => {});
    } catch (e) {}
}

const CORETAX_LOGIN_URL = 'https://coretaxdjp.pajak.go.id/identityproviderportal/Account/Login';
const DOWNLOAD_DIR = path.join(os.homedir(), 'Downloads', '.coretax-temp');
try { fs.mkdirSync(DOWNLOAD_DIR, { recursive: true }); } catch (e) {}
const PROFILE_ROOT = path.join(os.homedir(), '.coretax-agent-profile');

/** Empties DOWNLOAD_DIR (.coretax-temp). This is Playwright's own downloadsPath - every
 *  download made through any Chrome context this app launches lands here first with an
 *  internal GUID name. Automated flows (SPT/e-Bupot/MyBupot) fetch their PDFs directly via
 *  Coretax's API and never touch it at all; the only thing that leaves files here permanently
 *  is downloading something manually inside a "Login Coretax Manual" session (openCoretaxManual
 *  below) - nothing in the app ever reads them back out afterward, so they're safe to discard.
 *  Best-effort per-entry: a file still locked by an in-progress download is skipped rather than
 *  failing the whole cleanup, and just gets swept up next time. Called on startup and on quit
 *  (see main.js / gui/server.js's handleQuit). */
function clearDownloadTemp() {
    let entries;
    try { entries = fs.readdirSync(DOWNLOAD_DIR); } catch (e) { return; }
    let cleared = 0;
    for (const name of entries) {
        try { fs.rmSync(path.join(DOWNLOAD_DIR, name), { force: true, recursive: true }); cleared++; }
        catch (e) { /* still in use - leave it, next cleanup will catch it */ }
    }
    if (cleared) log('Membersihkan ' + cleared + ' file sisa unduhan lama dari .coretax-temp.');
}
...
// Chrome 152 can terminate with 0xC0000005 when Playwright 1.61 drives Coretax's blob: PDF
// download (confirmed live on BILLING_CODE_*.pdf). Bypass only that narrow browser-download
// path: retain the Blob in the page, send its bytes to Node, and save the identical PDF to the
// user's Downloads folder. No tab/window close behavior is changed.
const BILLING_BLOB_BRIDGE_INIT = `(() => {
    if (window.__coretaxAgentBillingBlobBridgeInstalled) return;
    const blobs = new Map();
    const delivering = new WeakSet();
    ...
    URL.createObjectURL = function (object) {
        ...
        if (object instanceof Blob) blobs.set(objectUrl, object);
        return objectUrl;
    };
    URL.revokeObjectURL = function (objectUrl) {
        setTimeout(() => blobs.delete(String(objectUrl)), 30000);
        return nativeRevokeObjectURL(objectUrl);
    };

    const captureBillingPdf = (anchor) => {
        if (!anchor || delivering.has(anchor)) return false;
        if (!/\/payment-portal\/[^/]+\/active-dashboard-billing-code/i.test(location.pathname)) return false;
        const filename = String(anchor.download || '');
        const objectUrl = String(anchor.href || '');
        const blob = blobs.get(objectUrl);
        if (!/^BILLING_CODE_[A-Za-z0-9._-]*\.pdf$/i.test(filename) || !blob) return false;
        delivering.add(anchor);
        void (async () => {
            try {
                ...
                await window.__ca_saveBillingBlobV116({ filename, base64: btoa(binary) });
            } catch (error) {
                try { console.error('[Coretax Agent] Gagal menyimpan Blob billing:', error); } catch (_) {}
            }
        })();
        return true;
    };
    ...
    HTMLAnchorElement.prototype.click = function () {
        if (captureBillingPdf(this)) return;
        return nativeAnchorClick.apply(this, arguments);
    };
    document.addEventListener('click', (event) => {
        const anchor = event.target && event.target.closest ? event.target.closest('a') : null;
        if (captureBillingPdf(anchor)) {
            event.preventDefault();
            event.stopImmediatePropagation();
        }
    }, true);
    window.__coretaxAgentBillingBlobBridgeInstalled = true;
})();`;

const _billingBlobBridgeContexts = new WeakSet();

async function installBillingBlobDownloadBridge(context, page) {
    if (!_billingBlobBridgeContexts.has(context)) {
        _billingBlobBridgeContexts.add(context);
        await context.exposeFunction('__ca_saveBillingBlobV116', async ({ filename, base64 }) => {
            const safeName = path.basename(String(filename || '')).replace(/[<>:"/\\|?*\x00-\x1f]/g, '_');
            if (!/^BILLING_CODE_[A-Za-z0-9._-]*\.pdf$/i.test(safeName)) throw new Error('Nama file billing tidak valid.');
            const data = Buffer.from(String(base64 || ''), 'base64');
            if (!data.length || data.length > 20 * 1024 * 1024) throw new Error('Ukuran PDF billing tidak valid.');
            const destination = path.join(os.homedir(), 'Downloads', safeName);
            await fs.promises.writeFile(destination, data);
            log('[Billing] PDF disimpan tanpa jalur Blob Chrome: ' + destination);
            return true;
        }).catch((e) => log('[Billing] Gagal memasang penyimpan Blob: ' + e.message));
        await context.addInitScript({ content: BILLING_BLOB_BRIDGE_INIT }).catch(() => {});
        context.on('page', (newPage) => newPage.evaluate(BILLING_BLOB_BRIDGE_INIT).catch(() => {}));
    }
    if (page && !page.isClosed()) await page.evaluate(BILLING_BLOB_BRIDGE_INIT).catch(() => {});
}
...
async function verifyImpersonationByCode(page, targetNpwp) {
    ...
    const code = `(() => {
        ...
        const sDigits = sub.replace(/\D/g, '');
        const wDigits = wantDigits.replace(/\D/g, '');
        if (!sDigits || !wDigits) return false;
        if (sDigits === wDigits) return true;
        if (sDigits.length >= 15 && wDigits.length >= 15 && sDigits.slice(0, 15) === wDigits.slice(0, 15)) return true;
        if (sDigits.includes(wDigits) || wDigits.includes(sDigits)) return true;
        return false;
    })()`;
    try {
...
async function openCoretaxManual(restricted = false, allowedEbupotSections = null) {
    ...
    try {
        const extraFlags = restricted ? ['--window-position=-32000,-32000'] : [];
        const context = await chromium.launchPersistentContext(userDataDir, {
            headless: false, channel: 'chrome', viewport: null,
            acceptDownloads: true, downloadsPath: DOWNLOAD_DIR,
            args: ANTI_NAG_FLAGS.concat(extraFlags), ignoreDefaultArgs: ['--enable-automation']
        });

        const watchdogScript = watchdog.buildWatchdogInitScript(allowedEbupotSections, false, restricted);
        await context.addInitScript({ content: watchdogScript }).catch(() => {});
        watchdog.installNetworkGuard(context, allowedEbupotSections, restricted, (url) => log('[SECURITY] Diblokir di lapisan jaringan: ' + url));
        ...
        await installBillingBlobDownloadBridge(context, page);
        // Sesi manual tidak pernah lewat loginAndImpersonate() (user login/impersonate sendiri
        // di jendela ini) - widget Lampiran perlu dipasang di sini juga, bukan cuma di jalur
        // otomatis, supaya tombolnya tetap muncul untuk pemakaian manual seperti ini.
        try { await lampiran.installLampiranWidget(page, {}); } catch (e) {}
...
/** Closes every Chrome window this process has opened - the per-PIC automation windows
 *  (_liveContexts) AND the manual-login session (manualSession), if any. These are launched via
 *  chromium.launchPersistentContext(), a genuinely separate OS process from this Node process -
 *  quitting Coretax Agent (process.exit()) does NOT touch them on its own, so any window left
 *  open (including a restricted editor's hidden off-screen one) survives as an orphan process
 *  after "quit" unless this is called first. Confirmed live 2026-07-27: the app's own dashboard/
 *  tray closed fine, but a Chrome window opened earlier for an impersonate test kept running. */
async function closeAllAutomationWindows() {
    const contexts = [...Array.from(_liveContexts.values()).map((c) => c.context)];
    if (manualSession && manualSession.context) contexts.push(manualSession.context);
    _liveContexts.clear();
    manualSession = null;
    await Promise.all(contexts.map((ctx) => ctx.close().catch(() => {})));
}

function getLiveEntityPage(picId) {
...
async function launchOrReuseContext(picId, onManualDownload, restricted = false, allowedEbupotSections = null) {
    const cached = _liveContexts.get(picId);
    if (cached && !cached.page.isClosed()) {
        await installBillingBlobDownloadBridge(cached.context, cached.page);
        log('Menggunakan jendela Chrome yang sudah terbuka untuk PIC ini (proses yang sama, tidak perlu buka/sambung ulang).');
        // lockImpersonate=false here on purpose: this window is about to go through switchToEntity
        // again (possibly to a DIFFERENT entity than whatever it's currently on) - locking the
        // switcher now would block the automation's own click before it gets a chance to run.
        // loginAndImpersonate re-locks it once a switch is actually confirmed. Safe because the
        // window is off-screen for the whole unlocked window (see hideWindowOffscreen just below).
        const watchdogScript = watchdog.buildWatchdogInitScript(allowedEbupotSections, false, restricted);
        await cached.context.addInitScript({ content: watchdogScript }).catch(() => {});
        await cached.page.evaluate(watchdogScript).catch(() => {});
        watchdog.installNetworkGuard(cached.context, allowedEbupotSections, restricted, (url) => log('[PERINGATAN KEAMANAN] Diblokir di lapisan jaringan: ' + url + ' (PIC: ' + picId + ')'));

        if (!restricted) {
            await cached.page.bringToFront().catch(() => { });
            bringChromeToFrontOnWindows();
        } else {
            await hideWindowOffscreen(cached.page);
        }
    ...
    log('Membuka Chrome (profil Coretax Agent, terpisah dari profil harian)...');
    log('[SECURITY] Status Restricted Editor: ' + (restricted ? 'AKTIF (Off-Screen & Watchdog Terkunci)' : 'TIDAK AKTIF (Mode Bebas/Admin)'));
    try {
        const extraFlags = restricted
        ...
        const context = await chromium.launchPersistentContext(userDataDir, {
            ...
            args: ANTI_NAG_FLAGS.concat(extraFlags),
            ignoreDefaultArgs: ['--enable-automation']
        });

        // lockImpersonate=false at initial launch, same reasoning as the reused-context branch
        // above: loginAndImpersonate's switchToEntity hasn't run yet, so the switcher must stay
        // usable BY THE AUTOMATION until a switch is confirmed, at which point it gets re-locked.
        const watchdogScript = watchdog.buildWatchdogInitScript(allowedEbupotSections, false, restricted);
        await context.addInitScript({ content: watchdogScript }).catch(() => {});
        watchdog.installNetworkGuard(context, allowedEbupotSections, restricted, (url) => log('[PERINGATAN KEAMANAN] Diblokir di lapisan jaringan: ' + url + ' (PIC: ' + picId + ')'));

        if (onManualDownload) context.on('download', (download) => { if (!downloadFlag.automated) return onManualDownload(download); });
        const page = context.pages()[0] || await context.newPage();
        await installBillingBlobDownloadBridge(context, page);
        if (restricted) await page.evaluate(watchdogScript).catch(() => {});
        _setupBlockedHitTracking(context, page, picId);

        if (!restricted) {
            await page.bringToFront().catch(() => { });
            bringChromeToFrontOnWindows();
        } else {
            await hideWindowOffscreen(page);
        }
        ...
        if (restricted) {
            throw new Error('Gagal membuka Chrome baru untuk akun terbatas: ' + launchErr.message);
        }
...
async function ensureLoggedIn(page, cred) {
    ...
    // Coretax 2026 tidak selalu menampilkan form kredensial di URL /Account/Login. Jika
    // sebuah sesi impersonasi masih aktif, halaman yang sama berubah menjadi "Pilih Akun"
    // dan hanya menyediakan tombol "Lanjutkan sebagai <entitas>" / "Masuk dengan akun lain".
    // Mencari #userid langsung pada state ini selalu gagal tiga kali, sehingga action berikutnya
    // untuk entitas lain pada PIC yang sama tidak pernah sempat login ulang. Pilih akun lain
    // terlebih dahulu agar kita kembali ke form PIC yang benar, bukan melanjutkan sebagai
    // entitas lama yang masih tersimpan pada session cookie.
    const initialUserField = await firstVisible(page, [page.locator('#userid'), page.getByPlaceholder(/NIK.*NPWP/i)], 1200);
    if (!initialUserField) {
        const useAnotherAccount = await firstVisible(page, [
            page.getByRole('button', { name: /Masuk dengan akun lain|Use another account/i }),
            page.getByText(/Masuk dengan akun lain|Use another account/i, { exact: true })
        ], 4000);
        if (useAnotherAccount) {
            log('Halaman Pilih Akun terdeteksi - memilih "Masuk dengan akun lain" agar dapat login sebagai PIC yang benar.');
            await useAnotherAccount.click({ noWaitAfter: true, timeout: 8000, force: true });
...
async function switchToEntity(page, cred, { npwp, name, individual, entityCode, userDataDir }) {
    ...
    // Fast path: check if target entity is ALREADY active before opening menu
    try {
        const alreadyActive = await verifyImpersonationByCode(page, npwp).catch(() => false);
        if (alreadyActive === true) {
            log('Entitas target ("' + target + '") SUDAH aktif di sesi ini (fast path).');
            return true;
        }
    } catch (e) {}
...
async function loginAndImpersonate(page, cred, entity, picId, opts = {}) {
    ...
    const checkpoint = opts.checkpoint || (async () => {});
    ...
    if (passphrase) {
        log('[Passphrase] Widget salin-passphrase dipasang untuk PIC "' + cred.pic_name + '" (muncul di pojok kanan bawah jendela Chrome).');
        try {
            const passScript = watchdog.buildPassphraseWidgetScript(passphrase);
            if (passScript) {
                await page.context().addInitScript({ content: passScript }).catch(() => {});
                await page.evaluate(passScript).catch(() => {});
            }
        } catch (e) {}
    }

    // Widget "Unduh Lampiran Lengkap" (automation/lampiran.js) - dipasang di sini, BUKAN di
    // masing-masing automation/*.js, supaya SETIAP login (SPT, e-Bupot, tombol Login biasa dsb)
    // otomatis dapat tombolnya begitu user membuka halaman view SPT secara manual sesudahnya -
    // sama persis alasan widget passphrase di atas dipasang di titik ini juga.
    try { await lampiran.installLampiranWidget(page, { entityCode: entity && entity.entity_id, restricted }); } catch (e) {}

    let hideInterval = null;
    if (restricted) {
        await hideWindowOffscreen(page);
        hideInterval = setInterval(() => {
            hideWindowOffscreen(page);
        }, 1500);
    }
    ...
    try {
        for (let attempt = 1; attempt <= ATTEMPTS; attempt++) {
            ...
            if (switched || entity.individual) {
                if (hideInterval) { clearInterval(hideInterval); hideInterval = null; }
                if (restricted) {
                    try {
                        const lockWatchdog = watchdog.buildWatchdogInitScript(allowedEbupotSections, true, restricted);
                        await page.context().addInitScript({ content: lockWatchdog }).catch(() => {});
                        await page.evaluate(lockWatchdog).catch(() => {});
                    } catch (e) {}
...
module.exports = {
    CORETAX_LOGIN_URL, DOWNLOAD_DIR, PROFILE_ROOT, downloadFlag, clearDownloadTemp,
    firstVisible, clickAltchaCheckbox, verifyImpersonationByCode,
    openCoretaxManual, getManualPage, getManualStatus, launchOrReuseContext, getLiveEntityPage,
    ensureLoggedIn, switchToEntity, loginAndImpersonate, isLoggedOut, revealWindow, hideWindowOffscreen, _sleep,
    closeAllAutomationWindows
};
