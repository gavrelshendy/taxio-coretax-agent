...
async function verifyImpersonationByCode(page, targetNpwp) {
    ...
    if (!want) return null;
    try {
        return await page.evaluate((wantDigits) => {
            const jwtSub = (token) => {
            ...
            // Finds the active access token's `sub` claim, wherever OIDC client keeps it (the
            // value may be a bare JWT, or a JSON blob with an "access_token" field - Coretax's
            // own oidc-client storage uses the latter under a key named after the client/issuer).
            const findSub = (store) => {
            ...
            if (sub === null) return null; // couldn't read the token at all -> caller falls back to a visual check
...
async function getActiveIdentity(page) {
    try {
        return await page.evaluate(() => {
            ...
            // Find the account name - the line at/next to the NPWP in the top account pill.
...
/** Launches a fresh Chrome window for this PIC, or (if one's already open and the profile
 *  lock rejects a second process) attaches to it over CDP. Returns {context, page, reused}. */
...
async function switchToEntity(page, cred, { npwp, name, individual, entityCode, userDataDir }) {
    ...
    await accountPill.evaluate(el => el.click()).catch(() => { });
    ...
    while (Date.now() < confirmDeadline) {
        const codeOk = await verifyImpersonationByCode(page, npwp);
    ...
    if (!confirmedBy) {
        ...
        const codeState = await verifyImpersonationByCode(page, npwp);
        if (codeState === null) {
