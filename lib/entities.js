...
// CONFIRMED LIVE 2026-08-13: relying solely on e.profile_type === 'Individual' missed a real
// case - PIC "FREDI SETYAWAN"'s own personal entity ("FREDI SETYAWAN, MR") had profile_type
// never explicitly set (a data-entry gap from whenever that entity was created, not a code
// bug), so a restricted editor could still search for, log into, and download from it. Falls
// back to comparing the entity's own name against its linked PIC's name when profile_type
// wasn't set at all, so a PIC's own personal account is caught even when nobody remembered to
// tag it - a corporate legal-form marker (PT/CV/etc) still rules it out either way, since small
// companies are often named after their founder.
function isLikelyIndividualEntity(e, picName) {
    if (e && e.profile_type === 'Individual') return true;
    if (e && e.profile_type) return false; // explicitly set to something else - trust it
    const name = String((e && e.entity_name) || '').trim();
    if (!name) return false;
    const nameUp = name.toUpperCase();
    if (/\b(PT|CV|YAYASAN|FIRMA|PERKUMPULAN|KOPERASI|PERSEROAN)\b/.test(nameUp)) return false;
    const picUp = String(picName || '').trim().toUpperCase();
    if (!picUp) return false;
    return nameUp === picUp || nameUp.startsWith(picUp + ',') || nameUp.startsWith(picUp + ' ');
}

/** CONFIRMED LIVE 2026-08-21: sejak Fase 40 (super-admin auto-join tiap grup baru), akun bisa
 *  jadi member LEBIH DARI SATU organisasi sekaligus (mis. akun super-admin gavrielshendy@,
 *  member di "Muliawati Group" DAN "Natasha Group") - query lama di sini cuma `.limit(1)` TANPA
 *  urutan sama sekali, jadi Postgres bebas balikin baris mana pun duluan. Ditemukan live: baris
 *  yang kepilih adalah grup yang SALAH (Muliawati Group, nyaris kosong) padahal grup yang
 *  sedang aktif dipakai user di Taxio Hub web (tabel `active_group`, sumber yang sama dipakai
 *  authHandleSession()-nya supabase.js) adalah Natasha Group - akibatnya daftar entitas Coretax
 *  Agent (Group maupun Me) muncul kosong/salah isi. Fix: utamakan org yang tercatat di
 *  active_group untuk user ini; baru fallback ke membership pertama kalau belum pernah pilih
 *  grup aktif sama sekali (akun benar-benar baru). */
async function getMyOrgId(client, userId) {
    const { data: activeRows, error: activeErr } = await client.from('active_group').select('org_id').eq('user_id', userId).limit(1);
    if (activeErr) throw activeErr;
    const activeOrgId = activeRows && activeRows[0] && activeRows[0].org_id;
    ...
    if ((!data || !data.length) && activeOrgId) {
        // Grup aktif tercatat tapi user ternyata bukan (lagi) member di situ (kasus langka,
        // mis. dikeluarkan dari grup) - fallback ke membership manapun yang masih ada.
        ({ data, error } = await client.from('memberships')
            .select('org_id, role, status, allowed_ebupot_sections, allowed_entity_ids')
...
async function listAutomatableEntities(client, orgId, currentUserId, hasOwnerColumn, mode) {
        ...
        if (links.length > 0) {
            for (const link of links) {
                ...
                out.push({
                    ...
                    npwp: e.npwp || '',
                    individual: isLikelyIndividualEntity(e, pic ? pic.pic_name : null),
                    pic_id: picId,
    ...
    // Explicit user request 2026-08-13/14: blocking Individual entities only at the point of
    // login/download (isIndividualEntityBlocked in gui/server.js) wasn't enough - a real test
    // showed the entity still fully visible/searchable/selectable in this list, which is itself
    // the problem ("yang pasti harus ilang" - must actually disappear, not just be rejected
    // after picking it). Filtered out here too, so a restricted editor never sees it at all.
    const result = userRole === 'restricted_editor' ? out.filter((r) => !r.individual) : out;
    result.sort((a, b) => a.entity_name.localeCompare(b.entity_name));
    return result;
}

/** Other Coretax PICs linked to this same entity, excluding the one already selected - used so
 *  SPT download can automatically retry under a co-PIC when a document was actually signed by
 *  them instead (e.g. an entity with 2 PICs like Fredi Setyawan + Ronald Tony - an SPT can only
 *  ever be fetched by whichever PIC actually signed it on Coretax's side). */
async function getOtherLinkedPicIds(client, orgId, entityId, excludePicId) {
    const { data, error } = await client.from('entity_coretax_pic_links').select('pic_id, is_primary').eq('org_id', orgId).eq('entity_id', entityId);
    if (error) { log('[Entities] Gagal memuat PIC lain untuk ' + entityId + ': ' + error.message); return []; }
    return (data || []).map((l) => l.pic_id).filter((id) => id && id !== excludePicId);
}
...
module.exports = { getMyOrgId, listAutomatableEntities, getCredential, getPassphrase, getOtherLinkedPicIds };
