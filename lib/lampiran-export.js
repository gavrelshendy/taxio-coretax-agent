...
async function writeWorkbook(sheets, meta, filePath){
    ...
    for(const sheet of sheets){
        ...
        for(const model of sheet.tables){
            ...
            for(const total of model.sourceTotals){const row=ws.addRow([total.label]);for(const [col,v]of Object.entries(total.values)){row.getCell(+col+1).value=numeric(v)??v;row.getCell(+col+1).numFmt=numberFormat(numeric(v));}row.font={name:'Calibri',size:10};}
            if(model.differences){const row=ws.addRow(['Selisih total hitung terhadap Coretax']);for(const[col,v]of Object.entries(model.differences)){row.getCell(+col+1).value=numeric(v);row.getCell(+col+1).numFmt=numberFormat(numeric(v));}row.fill={type:'pattern',pattern:'solid',fgColor:{argb:'FFFFE5DC'}};}
            ws.addRow([]);
...
async function renderTabs(tabs,meta,{mode='full',format='pdf',dir,stem,outputLayout='combined'}={}){
    const browser=await chromium.launch({channel:'chrome',headless:true});
    const sheets=[],buffers=[],paths=[];
    try{
        const page=await browser.newPage({viewport:{width:1172,height:800}});
...
module.exports={renderTabs,writeWorkbook,numeric,typed};
