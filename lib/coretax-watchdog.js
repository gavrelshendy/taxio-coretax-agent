...
function installNetworkGuard(context, allowedEbupotSections = null, isRestricted = true, onBlockedHit = null) {
    ...
    context.route('**/*', async (route) => {
        ...
        if (blacklist.some((pat) => urlLow.includes(pat))) {
...
function buildWatchdogInitScript(allowedEbupotSections = null, lockImpersonate = false, isRestricted = true) {
    ...
    return `(() => {
        ...
        function handleImpersonateBlock(e) {
            ...
            const matchedAccountEl = el.closest && el.closest('[class*="account"], [class*="profile"], [class*="impersonate"], [class*="avatar"], [class*="user"], app-account-switcher, app-header-profile, app-user-menu, .account-switcher, .user-info, .account-info, .cdk-overlay-container, .p-overlaypanel, .p-dialog');
            ...
            if (isAccountArea || isImpersonateText) {
                e.preventDefault();
                e.stopPropagation();
                e.stopImmediatePropagation();
                try {
                    document.querySelectorAll('.p-dialog, .p-overlaypanel, app-account-switcher, [class*="account-switcher"], [class*="impersonate-modal"], .cdk-overlay-container').forEach(m => {
        ...
        function isAccountSearchInput(el) {
            ...
            const area = el.closest && el.closest('[class*="account"], [class*="profile"], [class*="impersonate"], app-account-switcher, .account-switcher, .cdk-overlay-container, .p-overlaypanel, .p-dialog');
        ...
        function injectSecurityCSS() {
            ...
            let cssRules = \`
                ...
                html.ca-lock-impersonate-active .cdk-overlay-container,
                html.ca-lock-impersonate-active .p-overlaypanel,
                html.ca-lock-impersonate-active .p-dialog-mask,
                ...
                html.ca-lock-impersonate-active [class*="account"],
                html.ca-lock-impersonate-active [class*="profile"],
                html.ca-lock-impersonate-active [class*="impersonate"],
                html.ca-lock-impersonate-active [class*="avatar"],
                html.ca-lock-impersonate-active [class*="user"],
                html.ca-lock-impersonate-active app-account-switcher,
        ...
        window.disableBlockedElements = function() {
            ...
            const elements = document.querySelectorAll('a, button, [routerlink], li, div.card, .nav-item, .menu-item, mat-list-item, .cdk-overlay-container a, .cdk-overlay-container button, .cdk-overlay-container li, .cdk-overlay-container [role="menuitem"]');
            elements.forEach(el => {
            ...
            if (isImpersonateLocked()) {
                ...
                const pills = Array.from(document.querySelectorAll('.user-info, .account-info, app-account-switcher, .account-switcher, [class*="impersonate"], [class*="profile"], [class*="avatar"], app-header-profile, app-user-menu'))
                ...
                // Hide any open impersonate modals/dialogs in CDK overlay container
                try {
                    document.querySelectorAll('.p-dialog, .p-overlaypanel, app-account-switcher, [class*="account-switcher"], [class*="impersonate-modal"], .cdk-overlay-container').forEach(m => {
