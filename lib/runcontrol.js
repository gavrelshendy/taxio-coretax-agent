...
function start(label) {
    ...
    _resetFlags();
    run.active = true; run.label = label || ''; run.jenisRequested = []; run.jenisTally = {};
}
...
function status() { return { active: run.active, paused: run.paused, label: run.label, currentPageSize: run.currentPageSize, coretaxAs: run.coretaxAs, jenisRequested: run.jenisRequested, jenisTally: run.jenisTally }; }
...
module.exports = { start, finish, pause, pauseForDecision, resume, stop, skip, retry, back, checkpoint, status, setCoretaxAs, setPageSizeOverride, takePageSizeOverride, reportPageSize, setJenisRequested, recordRowDone, StopRunError, SkipComboError, RetryComboError, BackComboError };
